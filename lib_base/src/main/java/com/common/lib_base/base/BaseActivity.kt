                ...
                exception.flowWithLifecycle(lifecycle).collect { e ->
                    ...
                    when (e) {
                        is SocketTimeoutException -> ToastUtil.showShort(
                            this@BaseActivity,
                            getString(com.common.lib_base.R.string.request_time_out)
                        )

                        is ConnectException, is UnknownHostException -> ToastUtil.showShort(
                            this@BaseActivity,
                            getString(com.common.lib_base.R.string.network_error)
                        )

                        else -> ToastUtil.showShort(
                            this@BaseActivity,
                            e.message ?: getString(com.common.lib_base.R.string.response_error)
                        )
                    }
                }
            }
